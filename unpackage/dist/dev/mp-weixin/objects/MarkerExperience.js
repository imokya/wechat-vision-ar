...
var objects_Marker = require('./Marker.js')
const NEAR = 0.1
const FAR = 1e3
let instance = null
let DEBUG_SIZE = true
class MarkerExperience {
  constructor(canvas) {
    if (instance) {
      return instance
    }
  ...
  initVisionKit() {
    ...
    this.session.start((err) => {
      ...
      this.marker = new objects_Marker.Marker(this.session)
      ...
      const onFrame = (timestamp) => {
        const frame = this.session.getVKFrame(
          this.canvas.width,
          this.canvas.height
        )
        if (frame) {
  ...
  initTHREE() {
    const { camera, scene, renderer, GL, THREE, destroy } =
  ...
  copyModel() {
    ...
    const { scene, animations } = objects_loaders_gltfClone.cloneGltf(
      this.model,
      THREE
    )
